...
/* Private typedef -----------------------------------------------------------*/
typedef struct{
  uint16_t  CustomIcmserviceHdle;                    /**< icmService handle */
...
#define COPY_ICMSERVICE_UUID(uuid_struct)          COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x00,0xcc,0x7a,0x48,0x2a,0x98,0x4a,0x7f,0x2e,0xd5,0xb3,0xe5,0x8f)
...
void SVCCTL_InitCustomSvc(void)
{
   ...
   * service_max_attribute_record = 1 for icmService +
   *                                2 for Transmit +
   *                              = 3
   *
  ...
  ret = aci_gatt_add_service(UUID_TYPE_128,
                             ...
                             PRIMARY_SERVICE,
                             max_attr_record,
                             &(CustomContext.CustomIcmserviceHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_service command: icmService, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_service command: icmService \n\r");
  }
  ...
  ret = aci_gatt_add_char(CustomContext.CustomIcmserviceHdle,
                          UUID_TYPE_128, &uuid,
                          SizeTx,
...
tBleStatus Custom_STM_App_Update_Char(Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_TX:
      ret = aci_gatt_update_char_value(CustomContext.CustomIcmserviceHdle,
                                       CustomContext.CustomTxHdle,
                                       0, /* charValOffset */
                                       SizeTx, /* charValueLen */
                                       (uint8_t *)  pPayload);
